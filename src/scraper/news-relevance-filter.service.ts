...
const FOREIGN_BORDER_TERMS: { re: RegExp; label: string }[] = [
  ...
  { re: /파키스탄|방글라데시|카슈미르|아프가니스탄|인도[·\-]파|인도\s*(정부|군|총리|당국|북부|국경)/, label: '남아시아 국경' },
  { re: /미얀마|캄보디아|라오스|말레이시아/, label: '동남아 국경' },
  // '조지아'는 조지아주·조지아 커피와 겹치므로 국가 형태만 인정
  { re: /아르메니아|아제르바이잔|나고르노|키프로스|압하지야|조지아\s*(공화국|정부|국경)/, label: '캅카스·키프로스 분쟁' },
  { re: /이스라엘|가자\s*지구|팔레스타인|레바논|시리아|요르단|골란\s*고원/, label: '중동 국경·정전' },
  { re: /튀르키예|터키|그리스|쿠르드/, label: '튀르키예·그리스 국경' },
  // '수단'(手段)·'말리'(말리다)는 일반어와 겹치므로 국가 형태만 인정. 차드·니제르는 오매칭 우려로 제외
  { re: /콩고|르완다|에티오피아|에리트레아|리비아|알제리|튀니지|남수단|수단\s*(공화국|정부|내전|군|반군)|말리\s*(공화국|정부|내전|반군|북부)/, label: '아프리카 국경' },
];
